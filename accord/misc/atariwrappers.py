# Modified wrappers from stable-baselines
...
def make_atari_train(env_id):
    """
    Create a wrapped atari learning environment
...
def make_atari_eval(env_id):
    """
    Create a wrapped atari evaluation environment
    :param env_id: (str) the environment ID
    :return: (Gym Environment) the wrapped atari environment
    """
    env = gym.make(env_id)
    assert "NoFrameskip" in env.spec.id
    env = FrameMonitor(env, max_num_frames_per_episode=30 * 60 * 60)
    # Noop-regime
...
class FrameMonitor(gym.Wrapper):
    def __init__(self, env, max_num_frames_per_episode=30 * 60 * 60):
        gym.Wrapper.__init__(self, env)
        self.eprew = 0.0
        self.frame_count = 0
        self.max_frames = max_num_frames_per_episode
    ...
    def step(self, action):
        ...
        self.eprew += rew
        self.frame_count = self.env.unwrapped.ale.getEpisodeFrameNumber()
        if self.max_frames - 4 <= self.frame_count < self.max_frames:
            done = True
        if done:
            info["Game Over"] = True
