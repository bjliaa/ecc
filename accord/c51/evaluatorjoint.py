def evaluator(env_str, start, end, wids):
    ...
    from accord.agents.ensembles import JointEnsemble
    ...
    for step in np.array([i for i in range(start, end + 1)]) * savefreq:
        ...
        for wid in wids:
            names.append(
                glob.glob(f"models/{env_str}C51/step{step}/model-id-{wid}.h5")
                [0])
        print(names)
    ...
    ensemble = JointEnsemble(action_len, ensemblesize)
    ...
    tf.print(f"Saving scores for models {wids}...")

    data.reset_index(drop=True, inplace=True)
    data["Alg"] = "Joint"
    data["Model"] = f"{wids}"

    env_dir = env_str + "Joint"
